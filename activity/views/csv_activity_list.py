...
def csv_activity_list(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="activity_list.csv"'  
    act_list = Activity.objects.order_by('-id')  
    writer = csv.writer(response)  
    writer.writerow(['ID', 'Ticket', 'Job Receive Date', 'ECD', 'Client', 'DA', 'LUs', 'Wire Center', 'Job Type', 'Job No.', 'EWO', 'Client Ticket No.', 'Market', 'Revised ECD', 'PFP', 'Latitude', 'Longitude', 'F1 CFAS', 'F1 Job No.', 'F1 SOW', 'F2 CFAS', 'F2 Job', 'F2 SOW', 'Work Print', 'Footage', 'Terminals', 'Fielder', 'Work Start Date', 'Fielder ECD', 'Lead Eng', 'Notes For Fielders'])
